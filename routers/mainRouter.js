...
const { College, Stream, Amenity, Location, CollegeDescription, CollegeAmenities, CollegeLogo, CollegeCoverImage, CollegeYouTubeURL, CollegeGalleryImage, CollegeSEO, CollegeContact, CollegeCourse, CollegePlacement, CollegeRecruiters, User, Blog, Leads } = require('../models/model');
...
router.post('/api/college', upload.single('myFile'), async (req, res) => {
    try {
        ...
        if (results) {
            res.status(200).send({ message: 'College Created', response: results._id });
        }
        else {
...
router.get('/api/college', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All Colleges fetched Successfully', responses: allCollege });
    } catch (error) {
        console.log(error);
...
router.get('/api/parentstream', async (req, res) => {
    try {
        const data = ["Management", "Engineering", "Pharmacy", "Dental", "Education", "Journalism", "Law", "Medical", "Architecture", "Arts and Humanities", "Information Technology", "Commerce and Banking", "Hotel Management", "Design Colleges"]
        res.status(200).send({ message: 'All Parent Stream Fetched', responses: data });
    } catch (error) {
        console.log(error);
        res.status(200).send({ message: 'Internal Server Error' });
    }
...
router.get('/api/stream', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All Stream Fetched', response: allStream });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
...
router.get('/api/amenity', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All Amenity Fetched', response: allAmenities });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
...
router.get('/api/country', async (req, res) => {
    res.status(200).send({ message: 'Fetched All Countries', response: countries });
})
...
router.get('/api/location', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All Location Fetched', response: allLocation });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
...
router.post('/api/collegedescription', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All College Description Fetched', response: toSave });
    } catch (error) {
        console.log('this is in Catch : ', error)
        res.status(500).send({ message: 'Internal Server Error' });
    }
...
router.get('/api/collegedescription', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All College Description Fetched', response: allDesc });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
...
router.post('/api/collegeamenities', async (req, res) => {
    try {
        ...
        console.log('data registered');
        if (results) {
            res.status(200).send({ message: 'College Amenities Created', response: results });
        }
        else {
...
router.get('/api/collegeamenities', async (req, res) => {
    try {
        ...
        console.log(allAme)
        res.status(200).send({ message: 'All College Amenities Fetched', response: allAme });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
...
router.post('/api/collegegallery', upload.single('myFile'), async (req, res) => {
    try {
        console.log('inside try')
        console.log(req.body);
        const image = req.file ? '/mediaFiles/' + req.file.filename : null;
...
router.post('/api/collegevurl', async (req, res) => {
    try {
        ...
        if (results) {
            res.status(200).send({ message: 'College YouTube URL Saved', response: results });
        }
        else {
...
router.get('/api/course', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'All College Courses Fetched', response: allCourse });
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' });
    }
})
    ...
    try {
        ...
        if (isUserExist) {
            if (isUserExist.password == password) {
                res.status(200).cookie('authToken', 'df98klj5kljkl234lk3nelj32v4hj44h6hv8jcghxhg85v7kj5n45;l23klj46hjg57j45k6jjl', {
                    httpOnly: true,
...
router.get('/api/collegeinfo/:id', async (req, res) => {
    try {
        ...
        const collegeAmenities = await CollegeAmenities.find({ cid }).populate('amenities');
        ...
        res.status(200).send({
            message: 'Data Received', response: {
                college,
                collegeDescription,
                collegeAmenities,
                collegeLogo,
...
router.post('/api/blog', upload.single('myFile'), async (req, res) => {
    try {
        ...
        const toSave = Blog({ image: image, title: title, stream: stream, shortDescription: shortDescription, mainTitle: mainTitle, content: conContent });
        console.log('before save')
        console.log(toSave);
        const results = await toSave.save();
        console.log('data registered');
...
router.get('/api/graph', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'Graph Data Fetched', totalCollege: NumberOfCollege, totalUser: NumberOfUser, totalCourse: 25 })
    } catch (error) {
        res.status(500).send({ message: 'Internal Server Error' })
    }
})
